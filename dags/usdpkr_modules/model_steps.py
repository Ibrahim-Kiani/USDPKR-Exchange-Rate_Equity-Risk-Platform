...
TRANSFORMED_DATA_PATH = os.path.join(DATA_DIR, 'raw/engineered_features.csv')
SELECTED_FEATURES_PATH = os.path.join(OUTPUT_DIR, 'raw/selected_features.pkl')
...
def prepare_data(df=None, target_col='CPI', test_size=0.2):
    """
...
def train_lasso_model(selected_features_path=None, train_test_data_path=None, output_dir=None):
    ...
    try:
        ...
        # Save the model
        model_path = os.path.join(output_dir, 'usdpkr_lasso_model.pkl')
        ...
        metrics_path = os.path.join(output_dir, 'usdpkr_lasso_metrics.csv')
        ...
        try:
            ...
            y_full = df_full['USDPKR'] if 'USDPKR' in df_full.columns else df_full.iloc[:,0]
            ...
            pred_df.to_csv(os.path.join(output_dir, 'usdpkr_lasso_full_predictions.csv'), index=False)
            print('Full dataset predictions saved to usdpkr_lasso_full_predictions.csv')
        except Exception as e:
...
def train_stacked_model(selected_features_path=None, train_test_data_path=None, output_dir=None):
    ...
    try:
        ...
        # Save the model
        model_path = os.path.join(output_dir, 'usdpkr_stacked_model.pkl')
        with open(model_path, 'wb') as f:
        ...
        metrics_path = os.path.join(output_dir, 'usdpkr_stacked_metrics.csv')
        metrics_df.to_csv(metrics_path, index=False)
        ...
        try:
            ...
            pred_df.to_csv(os.path.join(output_dir, 'usdpkr_stacked_full_predictions.csv'), index=False)
            print('Full dataset predictions saved to usdpkr_stacked_full_predictions.csv')
        except Exception as e:
